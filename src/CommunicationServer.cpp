...
CommunicationServer::CommunicationServer() {
	...
	while(1) {
		...
		for(i = 0; i < FD_SETSIZE; ++i) {
			// if(is_paused_fd(i)) continue; // does not work
...
bool CommunicationServer::is_paused_fd(int fd) {
	for(int i = 0; i < paused_fds.size(); i++) {
		if(i == fd) {
			// printf("CommunicationServer::is_paused_fd(%i) = true\n", fd); 
			return true;
		}
	}
	// printf("CommunicationServer::is_paused_fd(%i) = false\n", fd);
	return false;
}
/*
	struct timeval timeout;
